...
const Customnode = ({ data, isConnectable, selected }) => {
  ...
  return (
      <RecoilRoot> 
          ...
          <div onClick={handleNodeClick}  style={{backgroundColor: `${colorGradient}`}} className={`relative  w-full h-full bg-gradient-to-b  border border-gray-300 rounded-lg  text-gray-800 min-h-24 min-w-24 shadow-lg overflow-hidden`}>
              ...
              <div className="z-0 grid place-items-center h-full text-xl ">
               <Progress value={progress} className="grid place-items-center h-2 w-4/5 "/>
               <HoverCard>
  <HoverCardTrigger className=' dark:text-white p-4 z-50 top-6 text-2xl'>{name}<HoverCardContent className="grid place-items-center w-96 shadow-lg border-slate-200">
  ...
  <List/>
              ...
              <Drawer>
  <DrawerTrigger className='text-xl  dark:text-white items-center relative bottom-2'>
  ...
  <DrawerContent>
    ...
    <Handle   id={`${colorGradient}`} type='source' position={Position.Right} isConnectable={isConnectable}  style={{ ...DEFAULT_HANDLE_STYLE,  background: `${colorGradient}` }} />
 </div>
